package dev.risas.notifications;
...
public final class Notification {
    ...
    public void render() {
        ...
        float x = (screenWidth) - (Math.max(CustomFont.getWidth(description), CustomFont.getWidth(name))) - 15;
        ...
        RenderUtil.roundedRectCustom(xVisual, yVisual - 3, sr.getScaledWidth() - xVisual, 30, 4f, new Color(0, 0, 0, 170), true, false, true, false);

        Gui.drawRect(xVisual + (percentageLeft * (CustomFont.getWidth(description)) + 8), yVisual + 25, screenWidth + 1, yVisual + 27, c.hashCode());

        final Color bright = new Color(Math.min(c.getRed() + 16, 255), Math.min(c.getGreen() + 35, 255), Math.min(c.getBlue() + 7, 255));

        CustomFont.drawStringBold(name, xVisual + 4, yVisual + 1, bright.getRGB());
        CustomFont.drawString(description, xVisual + 4, yVisual + 12, c.hashCode());
    }
