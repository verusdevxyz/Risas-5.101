...
import dev.risas.module.impl.player.Stealer;
...
import dev.risas.util.render.KeystrokeUtil;
...
public final class IngameGUI extends GuiIngame {
    ...
    private final TimeUtil timer3 = new TimeUtil();
    ...
    public static void onFadeOutline() {
        ...
        for (final Object m : modules) {
            ...
            if (c != Category.RENDER || showRender.isEnabled()) {
                ...
                if (mode.equals("Rise Blend") || mode.equals("Rise Christmas") || mode.equals("Rise 6 Old") || mode.equals("Rice") || mode.equals("Rise Cool") || mode.equals("Rise Sea") || mode.equals("Rise Cotton Candy") || mode.equals("Rise Blaze") || mode.equals("Rise Emo"))  {
    ...
    public static void onBlur() {
        ...
        if (modules == null || !mode.equals("Rise Blend") && !mode.equals("Rise Cotton Candy") && !mode.equals("Rise 6 Old") && !mode.equals("Rise Christmas") && !mode.equals("Rise Sea") && !mode.equals("Rise Cool") && !mode.equals("Rice") && !mode.equals("Never Lose") || mode.equals("Rise Blaze") || mode.equals("Rise Emo") ||  Minecraft.getMinecraft().gameSettings.showDebugInfo)
            return;
        ...
        switch (mode) {
            ...
            case "Classic Revamp": {
                ...
                for (int i = 0; i < name.length(); i++) {
                    ...
                    GlStateManager.pushMatrix();
                    GlStateManager.scale(1.5F, 1.5F, 1.5F);
                    mc.fontRendererObj.drawStringWithShadow(character, (float) (1.25 + off), 5, ThemeUtil.getThemeColorInt(i, ThemeType.LOGO));
                    GlStateManager.popMatrix();
                ...
                off = CustomFont.getWidthBig(name);
                mc.fontRendererObj.drawStringWithShadow(Rise.CLIENT_VERSION, (float) (0.25 + off), 5, ThemeUtil.getThemeColorInt(ThemeType.LOGO));

                if (shouldRenderCustomClientName) {
                    for (int i = 0; i < customName.length(); i++) {
                        final String character = String.valueOf(customName.charAt(i));
                        mc.fontRendererObj.drawStringWithShadow(character, (float) (0.25 + off), 13, ThemeUtil.getThemeColorInt(i, ThemeType.LOGO));
                        off += mc.fontRendererObj.getStringWidth(character);
                    }
                }
                break;
    ...
    private void renderArrayList() {
        ...
        if (timer3.hasReached(100) || modules == null || modules.isEmpty()){
            timer3.reset();
            ...
            if (c != Category.RENDER || showRender.isEnabled()) {
                ...
                switch (mode) {
                    ...
                    case "Classic Revamp":
                        ...
                        //final Color classicColor = (n instanceof Module) ? ((Module) n).getClassicThemeColor() : ((Script) n).getClassicThemeColor();
                        final Color classicColor = new Color((int) (Math.random() * 255), (int) (Math.random() * 255), (int) (Math.random() * 255));

                        mc.fontRendererObj.drawStringWithShadow(name, 2, renderY + 18, ThemeUtil.getThemeColorInt(moduleCount, ThemeType.ARRAYLIST));
