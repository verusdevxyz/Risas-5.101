...
import dev.risas.event.impl.other.CanPlaceBlockEvent;
...
import dev.risas.util.world.BlockUtil;
import dev.risas.util.world.EnumFacingOffset;
...
import net.minecraft.vialoadingbase.ViaLoadingBase;
...
//        }
    ...
    public void placeBlock() {
        ...
        if (!switchedSlot && ticksOnAir > placeDelay.getValue() + (randomisePlaceDelay.isEnabled() && !mc.gameSettings.keyBindJump.isKeyDown() ? Math.random() * 3 : 0) && ((BlockUtil.lookingAtBlock(blockFace, yaw, pitch, enumFacing.getEnumFacing(), rayCast.is("Strict"))) || this.rayCast.is("Off"))) {
            ...
            if (ViaLoadingBase.getInstance().getTargetVersion().isNewerThanOrEqualTo(ProtocolVersion.v1_13)) {
