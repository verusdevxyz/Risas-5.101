...
public class ChinaHat extends Module {
	...
	private final ModeSetting quality = new ModeSetting("Quality", this, "Normal", "Not china hat anymore", "Umbrella"
			, "Very Low", "Low", "Normal", "High", "Very High", "Smooth");
	...
	public void onRender3DEvent(final Render3DEvent event) {
        ...
        final double x = mc.thePlayer.lastTickPosX + (mc.thePlayer.posX - mc.thePlayer.lastTickPosX) * mc.timer
        .renderPartialTicks - mc.getRenderManager().viewerPosX;
        final double y = (mc.thePlayer.lastTickPosY + (mc.thePlayer.posY - mc.thePlayer.lastTickPosY) * mc.timer
        .renderPartialTicks - mc.getRenderManager().viewerPosY) + mc.thePlayer.getEyeHeight() + 0.5;
        final double z = mc.thePlayer.lastTickPosZ + (mc.thePlayer.posZ - mc.thePlayer.lastTickPosZ) * mc.timer
        .renderPartialTicks - mc.getRenderManager().viewerPosZ;
...
//
//            final double factor = (Math.sin(((System.currentTimeMillis() % 100000) / 100f) + (i / 69f) * 0.4f) + 1)
 * 0.5f;
//
//            RenderUtil.color(ColorUtil.mixColors(color1, color2, factor));

            GL11.glVertex3d(x, y, z);
            GL11.glVertex3d(x + radius * Math.cos(i * 6.283185307179586 / 4320), y - 0.24, z + radius * Math.sin(i *
            6.283185307179586 / 4320));
        }
		...
		final double rotations = rotate.isEnabled() ? ((mc.thePlayer.prevRenderYawOffset +
