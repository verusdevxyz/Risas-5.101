...
import dev.risas.util.Vector;
import dev.risas.util.math.MathUtil;
import dev.risas.util.math.RandomUtil;
import dev.risas.util.math.TimeUtil;
import dev.risas.util.player.*;
...
public final class Fly extends Module {
    ...
    private final BooleanSetting inf = new BooleanSetting("Infinite", this, false);
...
//    private final NumberSetting customSpeedH = new NumberSetting("Custom Speed Horizontal", this, 1, 0.1, 9.5, 0.1);
//    private final NumberSetting customSpeedV = new NumberSetting("Custom Speed Vertical", this, 1, 0.1, 9.5, 0.1);
//    private final NumberSetting customFall = new NumberSetting("Custom Fall Speed", this, 0, 0, 0.2, 0.005);
//    private final NumberSetting customTimer = new NumberSetting("Custom Timer", this, 1, 0.1, 5, 0.1);
//    private final ModeSetting customGround = new ModeSetting("Custom Ground State", this, "Real", "Real", "On Ground", "Off Ground");
//    private final ModeSetting customDamageMode = new ModeSetting("Custom Damage", this, "None", "None", "Fake", "Fall", "VClip");
//    private final BooleanSetting customBlink = new BooleanSetting("Custom Blink", this, false);
//    private final BooleanSetting customForceMovement = new BooleanSetting("Custom Force Movement", this, false);
//    private final BooleanSetting customLockY = new BooleanSetting("Custom Lock Y Input", this, false);
    ...
    private com.viaversion.viarewind.utils.math.Vector3d position;
    ...
    protected void onEnable() {
        ...
        // Good code?, Who cares.
        if (fakeDamage.isEnabled() && !(mode.is("Verus") || mode.is("Minemenclub") || mode.is("Old NCP") || mode.is("Bow Longjump") || mode.is("Minemenclub 2") || mode.is("Vulcan2") || mode.is("Damage") || mode.is("Wait for Damage") || mode.is("Bow Fly") || mode.is("ACR") || mode.is("Aac3") || mode.is("Taka"))) {
        ...
        switch (mode.getMode()) {
            ...
            case "Air Walk": {
                ...
                if (!firstEnable) {
                    firstEnable = true;
                    Rise.addChatMessage("Verus Air Lin.. Air Walks");
                }
            }
            case "Redesky":
                if (!firstEnable) {
    ...
    public void onUpdateAlwaysInGui() {
        ...
        flySpeed.hidden = !(mode.is("Vanilla") || mode.is("MineBox") || mode.is("Hypixel") || mode.is("Hypixel Fast Lag") || mode.is("Taka") || mode.is("Minemenclub 2") || mode.is("Minemenclub") || mode.is("Old NCP") || mode.is("Mush") || mode.is("Viper") || mode.is("Verus Lag") || mode.is("Verus") || mode.is("Collide") || mode.is("Creative") || mode.is("Damage") || mode.is("Hypixel Infinite") || mode.is("Hypixel Blink") || mode.is("Hypixel"));

        fakeDamage.hidden = (mode.is("Verus") || mode.is("Hypixel") || mode.is("Minemenclub") || mode.is("Custom") || mode.is("Old NCP") || mode.is("Bow Longjump") || mode.is("Minemenclub 2") || mode.is("Vulcan2") || mode.is("Damage") || mode.is("Wait for Damage") || mode.is("Bow Fly") || mode.is("ACR") || mode.is("Aac3") || mode.is("Taka"));
        ...
        timer.hidden = !(mode.is("Hypixel Blink") || mode.is("Old NCP") || mode.is("Bow Longjump"));
    ...
    public void onPreMotion(final PreMotionEvent event) {
        ...
        switch (mode.getMode()) {
            ...
            case "Block Drop": {
                mc.thePlayer.motionY = mc.gameSettings.keyBindJump.isKeyDown() ? 2 : mc.gameSettings.keyBindSneak.isKeyDown() ? -2 : 0;

                for (int i = 0; i < 3; ++i) {
                    PacketUtil.sendPacketWithoutEvent(new C03PacketPlayer.C06PacketPlayerPosLook(position.getX(), position.getY(), position.getZ(), rotation.getX(), rotation.getY(), false));
                }
                break;
            }
            ...
            case "Hypixel": {
                ...
                if (ticks == 11 && mc.thePlayer.onGround) {
                    hypixelStart = true;
                    fly = true;
                }
                if (bool) {
                ...
                if (!fly) {
                    if (PlayerUtil.isOnServer("hypixel")) {
                        event.setPitch((float) (89 + Math.random() / 100f));
                    }
                    ...
                    if (mc.thePlayer.posY > startingLocationY + 1) {
                        int slot = -1;

                        for (int i = 0; i < 9; i++) {
                            if (mc.thePlayer.inventory.getStackInSlot(i) != null &&
                                    mc.thePlayer.inventory.getStackInSlot(i).getItem() instanceof ItemBlock) {
                                slot = PlayerUtil.findSlimeBlock();
                                break;
                            }
                        }

                        if (slot == -1) {
                            this.registerNotification("You need a slime block to make fly work!");
                            this.toggleModule();
                            return;
                        }
                        ...
                        if (ticks > 5) {
                            ...
                            MovingObjectPosition movingObjectPosition = mc.thePlayer.rayTraceCustom(4.5f, mc.thePlayer.rotationYaw, 90.0F);
                            ...
                            if (mc.playerController.onPlayerRightClick(mc.thePlayer, mc.theWorld, mc.thePlayer.getHeldItem(), blockPos, enumFacing, hitVec)) {
...
//                    mc.thePlayer.motionY *= 0.9;
                    mc.thePlayer.motionY = 0;
                    MoveUtil.strafe(flySpeed.getValue() * Math.max(0.1, (400 - offGroundTicks) / 400d));
...
//                    Rise.addChatMessage("??");
            ...
            case "Bow Longjump": {
                ...
                if (mc.thePlayer.posY % 1 == 0 && mc.thePlayer.posY < mc.thePlayer.lastTickPosY) {
                    MoveUtil.stop();
                    this.toggleModule();
                    mc.timer.timerSpeed = 1;
                    return;
    ...
    public void onPostMotion(PostMotionEvent event) {
        ...
        if (mode.is("Block Drop")) {
            for (int i = 0; i < 1; ++i) {
                PacketUtil.sendPacketWithoutEvent(new C03PacketPlayer.C06PacketPlayerPosLook(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ, rotation.getX(), rotation.getY(), false));
            }
        }
    }
...
//            case "Custom": {
//
//                MoveUtil.strafe(customSpeedH.getValue());
//                if (!customLockY.isEnabled()) {
//                    mc.thePlayer.motionY = (mc.gameSettings.keyBindJump.isKeyDown() ? customSpeedV.getValue() : mc.gameSettings.keyBindSneak.isKeyDown() ? -customSpeedV.getValue() : 0) - customFall.getValue();
//                } else {
//                    mc.thePlayer.motionY = -customFall.getValue();
//                }
//
//                break;
//            }
            ...
            case "Hypixel": {
                MoveUtil.strafe();
                if (!hypixelStart || !MoveUtil.isMoving()) {
                    MoveUtil.setMoveEventSpeed(event, .00);
                }
    ...
    public void onPacketSend(final PacketSendEvent event) {
        ...
        switch (mode.getMode()) {
            case "Hypixel": {
//                mc.timer.timerSpeed = (float) (0.5f + (Math.sin(offGroundTicks) + 1) / 8f);
                if (p instanceof C03PacketPlayer && setback && fly && stage > 10) {
                    ((C03PacketPlayer) p).setOnGround(true);
                }
            ...
            case "Block Drop":
                if (p instanceof C03PacketPlayer) {
                    ...
                    PacketUtil.sendPacketWithoutEvent(new C03PacketPlayer.C06PacketPlayerPosLook(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ, mc.thePlayer.rotationYaw, mc.thePlayer.rotationPitch, false));
                }
                break;
    ...
    public void onPacketReceive(final PacketReceiveEvent event) {
        ...
        if (mode.getMode().equals("Hypixel") && p instanceof S08PacketPlayerPosLook) setback = true;

        if (mode.getMode().equals("Hycraft") && p instanceof S08PacketPlayerPosLook) {
            if (!flag) {
                ...
                if (Math.sqrt(deltaX * deltaX + deltaY * deltaY + deltaZ * deltaZ) < 10) {
                    event.setCancelled(true);
                    PacketUtil.sendPacketWithoutEvent(new C03PacketPlayer.C06PacketPlayerPosLook(packet.getX(), packet.getY(), packet.getZ(), packet.getYaw(), packet.getPitch(), false));
                }
            }
        }
